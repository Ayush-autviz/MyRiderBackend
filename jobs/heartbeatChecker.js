...
const startHeartbeatChecker = (io) => {
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      console.log(`Checking heartbeat for ${onlineDrivers.length} online drivers`);
      ...
      console.log(`Found ${inactiveDrivers.length} inactive drivers to force offline`);
      ...
      const forceOfflinePromises = inactiveDrivers.map(async (driverInfo) => {
        try {
          ...
          io.to(`driver_${driverInfo.id}`).emit("forceOffline", { 
            message: "Inactivity timeout - You have been automatically taken offline" 
          });

          console.log(`Driver ${driverInfo.name} (${driverInfo.id}) forced offline due to ${driverInfo.timeDiff} minutes of inactivity`);
          
          return driverInfo;
        } catch (error) {
          console.error(`Error forcing driver ${driverInfo.id} offline:`, error);
          return null;
        }
      });
      ...
      console.log(`Successfully forced ${successCount}/${inactiveDrivers.length} inactive drivers offline`);
