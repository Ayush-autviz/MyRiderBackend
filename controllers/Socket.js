      ...
      socket.on('acceptRide', async (rideId) => {
        try {
          ...
          if (!driver.liveRequests.some(req => req.rideId.equals(rideId))) {
            socket.emit('error', { message: 'Ride request not assigned to you' });
            return;
          }
          ...
          socket.to(`customer_${ride.customer}`).emit('rideAccepted', {
            rideId,
            driverId: user.id,
            driver: await Driver.findById(user.id).select('firstName lastName vehicleDetails currentLocation'),
          });

          // Subscribe customer to driver's location
          socket.to(`customer_${ride.customer}`).emit('subscribeToDriverLocation', user.id);
          ...
          nearbyDrivers.forEach(async (nearbyDriver) => {
            socket.to(`driver_${nearbyDriver._id}`).emit('rideTaken', { rideId });
      ...
      const checkHeartbeat = setInterval(async () => {
        try {
          ...
          if (driver && driver.isAvailable && driver.lastHeartbeat) {
            const timeDiff = (new Date() - new Date(driver.lastHeartbeat)) / 1000 / 60; // minutes
            if (timeDiff > 10) {
              await Driver.findByIdAndUpdate(user.id, {
                isAvailable: false,
                lastHeartbeat: null,
    ...
    if (user.role === 'customer') {
      ...
      socket.on('bookRide', async (rideId) => {
        try {
          ...
          if (ride.customer.toString() !== user.id) {
            socket.emit('error', { message: 'Unauthorized: Ride does not belong to this customer' });
            return;
          }
          if (ride.status !== 'searchingDriver') {
            socket.emit('error', { message: 'Ride is not in searchingDriver status' });
            return;
          }
          ...
          if (nearbyDrivers.length === 0) {
            ...
            await ride.save();
            socket.emit('noDriversAvailable', { message: 'No drivers available nearby' });
      ...
      socket.on('searchNearbyDrivers', async (pickupCoords) => {
        try {
          ...
          console.log(`Found ${nearbyDrivers.length} drivers nearby for ${user.id}`);
        } catch (error) {
